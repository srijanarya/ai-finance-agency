...
export class MarketDataController {
  ...
  async getRealtimeData(@Param('symbol') symbol: string): Promise<MarketData> {
    try {
      const data = await this.marketDataService.getRealtimeData(symbol.toUpperCase());
      
      if (!data) {
        throw new HttpException('Market data not found for symbol', HttpStatus.NOT_FOUND);
      }
  ...
  @ApiResponse({ status: 200, description: 'Market data retrieved successfully', type: [MarketData] })
  async getBatchRealtimeData(@Body() body: { symbols: string[] }): Promise<MarketData[]> {
    try {
      ...
      if (body.symbols.length > 50) {
        throw new HttpException('Maximum 50 symbols allowed per request', HttpStatus.BAD_REQUEST);
      }

      const normalizedSymbols = body.symbols.map(s => s.toUpperCase());
      return await this.marketDataService.getMultipleRealtimeData(normalizedSymbols);
    } catch (error) {
      throw new HttpException(
  ...
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false, description: 'Time interval' })
  ...
  @Get('ohlc/:symbol')
  @ApiOperation({ summary: 'Get OHLC (Open, High, Low, Close) data for a symbol' })
  ...
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  @ApiQuery({ name: 'period', required: false, description: 'RSI period (default: 14)' })
  @ApiQuery({ name: 'days', required: false, description: 'Number of days for analysis' })
  async getRSI(
    @Param('symbol') symbol: string,
    @Query('interval') interval: TimeInterval = TimeInterval.ONE_DAY,
  ...
  @ApiOperation({ summary: 'Get MACD (Moving Average Convergence Divergence) for a symbol' })
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  ...
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  @ApiQuery({ name: 'period', required: false, description: 'Period (default: 20)' })
  @ApiQuery({ name: 'days', required: false })
  ...
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  @ApiQuery({ name: 'type', enum: ['SMA', 'EMA'], required: false, description: 'MA type' })
  @ApiQuery({ name: 'period', required: false, description: 'Period (default: 20)' })
  @ApiQuery({ name: 'days', required: false })
  ...
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  @ApiQuery({ name: 'period', required: false, description: 'Period (default: 14)' })
  @ApiQuery({ name: 'days', required: false })
  async getStochastic(
    @Param('symbol') symbol: string,
    @Query('interval') interval: TimeInterval = TimeInterval.ONE_DAY,
  ...
  @ApiOperation({ summary: 'Get comprehensive technical analysis for a symbol' })
  @ApiQuery({ name: 'interval', enum: TimeInterval, required: false })
  @ApiQuery({ name: 'days', required: false })
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Number of records (max 1000)' })
  async getMarketDataHistory(
    @Param('symbol') symbol: string,
    @Query('limit') limit?: number,
  ) {
    try {
      const recordLimit = Math.min(limit ? parseInt(limit.toString(), 10) : 100, 1000);
