...
export class AlertController {
  ...
  async getAlertsBySymbol(
    ...
    try {
      return await this.alertService.getAlertsBySymbol(
        symbol.toUpperCase(),
        status,
      );
    } catch (error) {
  ...
  async createBatchAlerts(
    ...
    try {
      ...
      if (
        !Array.isArray(createAlertsDto.alerts) ||
        createAlertsDto.alerts.length === 0
      ) {
        throw new HttpException(
          "Alerts array is required",
      ...
      if (createAlertsDto.alerts.length > 20) {
        throw new HttpException(
          "Maximum 20 alerts allowed per batch",
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  async bulkUpdateAlerts(
    @Body() updateDto: { alertIds: string[]; updates: UpdateAlertDto },
  ) {
    try {
      if (
        !Array.isArray(updateDto.alertIds) ||
        updateDto.alertIds.length === 0
      ) {
        throw new HttpException(
          "Alert IDs array is required",
          HttpStatus.BAD_REQUEST,
        );
      }
      ...
      if (updateDto.alertIds.length > 50) {
        throw new HttpException(
          "Maximum 50 alerts allowed per bulk update",
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  async bulkDeleteAlerts(@Body() deleteDto: { alertIds: string[] }) {
    try {
      if (
        !Array.isArray(deleteDto.alertIds) ||
        deleteDto.alertIds.length === 0
      ) {
        throw new HttpException(
          "Alert IDs array is required",
          HttpStatus.BAD_REQUEST,
      ...
      if (deleteDto.alertIds.length > 50) {
        throw new HttpException(
          "Maximum 50 alerts allowed per bulk delete",
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  @ApiResponse({
    status: 200,
    description: "Active alert count retrieved successfully",
  })
  async getActiveAlertCount(@Request() req: any) {
    try {
      ...
      const activeAlerts = await this.alertService.getUserAlerts(
        userId,
        AlertStatus.ACTIVE,
      );
