...
import { WatchlistWithMarketDataDto } from '../dto/watchlist-response.dto';
...
export class WatchlistController {
  ...
  async getUserWatchlist(@Request() req: any): Promise<WatchlistWithMarketData[]> {
    try {
  ...
  @ApiResponse({ status: 201, description: 'Symbol added to watchlist successfully', type: Watchlist })
  @ApiResponse({ status: 400, description: 'Invalid request data or symbol already in watchlist' })
  ...
  @ApiResponse({ status: 200, description: 'Watchlist symbols retrieved successfully' })
  async getUserWatchlistSymbols(@Request() req: any): Promise<{ symbols: string[] }> {
    try {
      const userId = req.user?.sub;
      if (!userId) {
        throw new HttpException('User authentication required', HttpStatus.UNAUTHORIZED);
      ...
      const symbols = await this.watchlistService.getUserWatchlistSymbols(userId);
      return { symbols };
    } catch (error) {
      throw new HttpException(
  ...
  @ApiResponse({ status: 200, description: 'Watchlist statistics retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Filtered watchlist retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Watchlist item retrieved successfully' })
  @ApiResponse({ status: 404, description: 'Watchlist item not found' })
  async getWatchlistItem(@Param('id') id: string): Promise<WatchlistWithMarketData> {
    try {
      const item = await this.watchlistService.getWatchlistItem(id);
      
      if (!item) {
        throw new HttpException('Watchlist item not found', HttpStatus.NOT_FOUND);
      }
  ...
  @ApiResponse({ status: 200, description: 'Watchlist item updated successfully', type: Watchlist })
  @ApiResponse({ status: 404, description: 'Watchlist item not found' })
  async updateWatchlistItem(
    ...
    try {
      return await this.watchlistService.updateWatchlistItem(id, updateWatchlistDto);
    } catch (error) {
      throw new HttpException(
  ...
  @ApiResponse({ status: 200, description: 'Symbol removed from watchlist successfully' })
  @ApiResponse({ status: 404, description: 'Watchlist item not found' })
  async removeFromWatchlist(@Param('id') id: string): Promise<{ message: string }> {
    try {
  ...
  async reorderWatchlist(
    ...
    try {
      ...
      if (!Array.isArray(reorderDto.itemOrders)) {
        throw new HttpException('itemOrders array is required', HttpStatus.BAD_REQUEST);
      }

      await this.watchlistService.reorderWatchlist(userId, reorderDto.itemOrders);
  ...
  async bulkAddToWatchlist(
    ...
    try {
      ...
      if (!Array.isArray(bulkAddDto.symbols) || bulkAddDto.symbols.length === 0) {
        throw new HttpException('Symbols array is required', HttpStatus.BAD_REQUEST);
      }

      if (bulkAddDto.symbols.length > 50) {
        throw new HttpException('Maximum 50 symbols allowed per bulk add', HttpStatus.BAD_REQUEST);
      }

      const results = await this.watchlistService.bulkAddToWatchlist(userId, bulkAddDto.symbols);
  ...
  async importWatchlistFromCSV(
    ...
    try {
      ...
      const result = await this.watchlistService.importWatchlistFromCSV(userId, importDto.csvData);
  ...
  async exportWatchlistToCSV(@Request() req: any): Promise<{ csv: string; filename: string }> {
    try {
      const userId = req.user?.sub;
      if (!userId) {
        throw new HttpException('User authentication required', HttpStatus.UNAUTHORIZED);
      ...
      // Create CSV content
      const headers = ['Symbol', 'Display Name', 'Notes', 'Tags', 'Current Price', 'Change %', 'Added At Price'];
  ...
  async bulkRemoveFromWatchlist(
    @Body() bulkDeleteDto: { itemIds: string[] }
  ) {
    try {
      if (!Array.isArray(bulkDeleteDto.itemIds) || bulkDeleteDto.itemIds.length === 0) {
        throw new HttpException('Item IDs array is required', HttpStatus.BAD_REQUEST);
      }

      if (bulkDeleteDto.itemIds.length > 50) {
        throw new HttpException('Maximum 50 items allowed per bulk delete', HttpStatus.BAD_REQUEST);
      }
