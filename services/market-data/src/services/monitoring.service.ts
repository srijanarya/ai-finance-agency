...
export class MonitoringService {
  ...
  private async getActiveSymbols(): Promise<string[]> {
    const recentData = await this.marketDataRepository
      ...
      .where('timestamp > :date', { date: new Date(Date.now() - 24 * 60 * 60 * 1000) })
      .getRawMany();
  ...
  async getHealthStatus(): Promise<{
    ...
    if (metrics.connections.websocket === 0) {
      checks.websocket = { status: 'degraded', message: 'No WebSocket connections' };
    }
    ...
    if (Object.values(checks).some(c => c.status === 'unhealthy')) {
      status = 'unhealthy';
    } else if (Object.values(checks).some(c => c.status === 'degraded') || activeAlerts.length > 5) {
      status = 'degraded';
    }
