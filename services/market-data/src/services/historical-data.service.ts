...
import { HistoricalData, TimeInterval } from '../entities/historical-data.entity';
...
export class HistoricalDataService {
  ...
  async getHistoricalData(query: HistoricalDataQuery): Promise<HistoricalData[]> {
    try {
      ...
      const isRecent = !query.endDate || query.endDate > new Date(Date.now() - 24 * 60 * 60 * 1000);
  ...
  async saveHistoricalData(candles: CandleData[], source: DataSource): Promise<HistoricalData[]> {
    try {
      ...
      await this.clearHistoricalDataCache(candles[0]?.symbol, candles[0]?.interval);
  ...
  async fetchDailyHistoricalData(): Promise<void> {
    try {
      ...
      this.logger.error('Error in scheduled daily historical data fetch:', error);
    }
  }
  ...
  async fetchMinuteHistoricalData(): Promise<void> {
    try {
      ...
      this.logger.error('Error in scheduled minute historical data fetch:', error);
    }
  }
  ...
  private async fetchAndSaveHistoricalData(
    ...
    try {
      ...
      if (!data || data.length === 0) {
        ...
        data = await this.fetchHistoricalFromAlphaVantage(symbol, interval, days);
        source = DataSource.ALPHA_VANTAGE;
      }

      if (data && data.length > 0) {
        await this.saveHistoricalData(data, source);
        this.logger.debug(`Fetched ${data.length} historical data points for ${symbol} (${interval})`);
      }
    } catch (error) {
  ...
  private async fetchHistoricalFromYahoo(
    ...
    try {
      ...
      this.logger.debug(`Yahoo Finance historical error for ${symbol}:`, error.message);
      return [];
    }
  }
  ...
  private async fetchHistoricalFromAlphaVantage(
    ...
    try {
      ...
      this.logger.debug(`Alpha Vantage historical error for ${symbol}:`, error.message);
      return [];
    }
  }
  ...
  private async clearHistoricalDataCache(symbol: string, interval: TimeInterval): Promise<void> {
    try {
