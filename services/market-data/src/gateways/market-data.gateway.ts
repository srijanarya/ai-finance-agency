...
export class MarketDataGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    try {
      ...
      if (token) {
        try {
          ...
          this.logger.warn('Invalid token provided during WebSocket connection');
        }
      }
  ...
  async handleSubscribe(
    ...
    try {
      ...
      if (symbols.length > 50) {
        client.emit('error', { message: 'Too many symbols. Maximum 50 allowed.' });
        return;
      }
      ...
      const currentData = await this.marketDataService.getMultipleRealtimeData(subscribedSymbols);
      ...
      this.logger.log(`Client ${client.id} subscribed to ${subscribedSymbols.length} symbols`);
    } catch (error) {
  ...
  async handleUnsubscribe(
    ...
    try {
      ...
      this.logger.log(`Client ${client.id} unsubscribed from ${unsubscribedSymbols.length} symbols`);
    } catch (error) {
  ...
  async handleSubscribeWatchlist(
    ...
    try {
      if (!client.userId) {
        client.emit('error', { message: 'Authentication required for watchlist subscription' });
        return;
      }

      // Get user's watchlist symbols
      const watchlistSymbols = await this.watchlistService.getUserWatchlistSymbols(client.userId);
      ...
      this.logger.log(`Client ${client.id} subscribed to watchlist (${subscribedSymbols.length} symbols)`);
    } catch (error) {
  ...
  async handleGetMarketData(
    ...
    try {
      ...
      const marketData = await this.marketDataService.getRealtimeData(symbol.toUpperCase());
  ...
  handleMarketDataUpdate(payload: { symbol: string; data: MarketData }) {
    try {
      ...
      if (subscribedClients && subscribedClients.size > 0) {
        ...
        this.logger.debug(`Broadcasted market data update for ${symbol} to ${subscribedClients.size} clients`);
      }
    } catch (error) {
  ...
  handleAlertTriggered(payload: { alert: any; marketData: MarketData }) {
    try {
      ...
      const client = Array.from(this.connectedClients.values()).find(c => c.userId === alert.userId);
  ...
  broadcastMarketStatus(status: { isOpen: boolean; session: string; message?: string }) {
  ...
  getConnectionStats() {
    return {
      ...
      subscriptionDetails: Array.from(this.symbolSubscriptions.entries()).map(([symbol, clients]) => ({
        symbol,
